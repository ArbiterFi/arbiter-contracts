...
interface IArbiterAmAmmHarbergerLease {
    ...
    function minimumRentBlocks(PoolKey calldata key) external view returns (uint64);
    ...
    function transitionBlocks(PoolKey calldata key) external view returns (uint64);
    ...
    /// @param key The key of the pool to check
    function currentRentPerBlock(PoolKey calldata key) external view returns (uint96);

    /// @return The block number of the last rent payment
    /// @param key The key of the pool to check
    function currentRentEndBlock(PoolKey calldata key) external view returns (uint32);
